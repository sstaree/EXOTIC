...
parser.add_argument('--lrkl', type=float, default=0.001)
parser.add_argument('--lrkl1', type=float, default=0.001)
...
parser.add_argument('--miss_rate', default=0.9, type=float)
...
def train_DIC(mul_X, mul_X_val, WE, WE_val, yt_label_noisy, yv_label, yt_label_clean, yv_label_clean, device, args):
    ...
    for epoch in range(int(args.epochs)):
        ...
        orig_label = np.argmax(yt_label_correct, axis=1)
        ...
        if epoch<args.warm_epoch:
            ...
            true_label = np.argmax(yt_label_clean, axis=1)
            orig_label = np.argmax(yt_label_correct, axis=1)
            mask_equal = np.equal(noisy_label, true_label)
            mask_nequal = np.not_equal(noisy_label, true_label)
            right_rate = np.equal(true_label, np.argmax(yt_label_correct, axis=1))
            prob_equal = np.sort(prob[mask_equal])
            prob_neque = np.sort(prob[mask_nequal])
            prob_thred1 = prob_equal > args.threshold_sver
            prob_thred2 = prob_neque > args.threshold_sver
            # print("select clean:{}/{}".format(np.sum(prob_thred1), prob_equal.shape[0]))
            # print("select noisy:{}/{}".format(np.sum(prob_thred2), prob_neque.shape[0]))
...
def main():
    ...
    WE = get_mask(num_view,num_data,args.miss_rate)


    for i in range(WE.shape[0]):
        for j in range(num_view):
            if WE[i][j]==0:
                X[j][i] = np.zeros(X[j].shape[1])+np.nan
    path = "{}".format(args.dataset)
    if not os.path.exists(path):
        os.mkdir(path)
    with open("{}\X_{}_{}.pkl".format(path,args.dataset,args.miss_rate), "wb") as f:
        pickle.dump(X, f, protocol=pickle.HIGHEST_PROTOCOL)  # 设置为可用的最高协议
    return
    # WE = folds_data
    # print(WE[1000])
    # print(X[0][1000])
...
def single_dataset_ratio(dataset,noise):
    filename = "./result/{}.txt".format(dataset)
    zz = [2024]
    # zz = [2024]
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        ...
        for mi in miss:
            args.miss_rate = mi
            single_dataset_ratio(dataset,0)
